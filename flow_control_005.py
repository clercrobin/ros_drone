...
class FlowController:
    ...
    def apply_control(self, next, image_out):
       ...
       # print("right : " + str(right_flow))
        ...
        if(time.clock() - self.time_since_too_near > self.pause):
            ...
            if (abs(currentmoy)<0.05*self.moy[0]):
                self.go_twist.linear.x = -0.4  
                self.time_since_too_near = time.clock()
                self.go_twist.angular.z = max_angular_twist/3
            elif (abs(currentmoy)<0.1*self.moy[0]):
                self.go_twist.linear.x = 0.1
                #self.time_since_too_near = time.clock()+0.9*self.pause
